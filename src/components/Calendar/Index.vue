 		...
 		<div class="m" v-for="item,monthIndex in Callist">
 			...
 			<table class='dateTable'>
 				<tr v-for="row,index in Math.ceil(item.trData.length/7)">
 					<td v-for="col,i in days" @click="selectDate(monthIndex,item.trData[index*7+i],$event)" :data-set="item.year+'-'+item.month+'-'+item.trData[index*7+i]" v-bind:class="{'active today':item.trData[index*7+i]==today&&monthIndex==0,'gray':item.trData[index*7+i]<today&&monthIndex==0}">{{ item.month+"-"+item.trData[index*7+i] | dateText }}</td>
 				</tr>
 			</table>
 		</div>
 	</div>
